...
def analyze_and_pickup():
    ...
    try:
        for idx, filepath in enumerate(sgf_files, start=1):
            ...
            # 条件判定
            # ① 1手ごとの勝率急落幅（Winrate Drop）: 15%〜30% が存在するか (0.15 <= drop <= 0.30 または 0.15以上)
            # ② 目数損失幅（Score Loss）: 6目〜10目以上の損失が存在するか (score_loss >= 6.0)
            ...
            for t in range(1, total_moves + 1):
                ...
                # ① 勝率急落幅: 15%以上 (0.15 <= winrate_drop)
                if winrate_drop >= 0.15:
                    cond1_hits.append({
                        "turn": t,
            ...
            if is_matched:
                ...
                print(f"     Cond1 (WinrateDrop>=15%): {len(cond1_hits)}, Cond2 (ScoreLoss>=6): {len(cond2_hits)}, Cond3 (Turnaround 70%->40%): {len(cond3_hits)}")
